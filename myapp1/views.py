...
def delete_contact(request, contact_id): #DELETE
    ...
    try:
        ...
        return redirect('index')
    today = date.today()
    age = today.year - contact_sel.date_of_birth.year - ((today.month, today.day) < (contact_sel.date_of_birth.month, contact_sel.date_of_birth.day))
    if age < 18:
        return HttpResponse("cannot delete you are young")
    contact_sel.delete()
    return redirect('index')